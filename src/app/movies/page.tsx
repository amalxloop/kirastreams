...
import { getPopular, getTopRated, getNowPlaying, getMoviesByGenre, getImageUrl, TMDBMovie, TMDB_GENRES } from "@/lib/tmdb";
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(1200px_600px_at_20%_-10%,rgba(124,58,237,0.25),transparent),radial-gradient(1200px_600px_at_80%_10%,rgba(59,130,246,0.2),transparent)]">
      ...
      <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b border-border/40">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-14 sm:h-16 lg:h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 sm:gap-3">
            <div className="relative h-8 w-8 sm:h-10 sm:w-10 lg:h-12 lg:w-12 rounded-md overflow-hidden shadow-[0_0_24px_2px_rgba(167,139,250,0.45)]">
              <Image
            ...
            <span className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold tracking-wide text-transparent bg-clip-text bg-gradient-to-r from-violet-300 via-fuchsia-300 to-sky-300">
              KiraStreams
            </span>
          </Link>
          <nav className="hidden md:flex items-center gap-6">
            <Link href="/movies" className="text-sm text-foreground font-medium">
              Movies
            </Link>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Discover the latest and greatest films</p>
          </div>
        </div>
        ...
        {/* Movies Grid */}
        {loading && <p className="text-sm text-muted-foreground">Loading movies...</p>}
